...
ARTIFACT_FILTER='*'
ros_RELEASE='13'
REPO_NAME='ros_reg_'$ros_RELEASE
ros_REG_NAME='ros_reg_'$(cat  /etc/machine-id)
ros_REG_URL='http://51.145.162.94:5000'
...
ROS_REPOS=$(curl -sk -X GET $ros_REG_URL'/v2/_catalog' | jq '.repositories[]' | tr -d '"')
ROS_REPOS_CNT=$(echo "$ROS_REPOS" | wc -l)
c=1
for r in $ROS_REPOS; do
    CONT_REPO_NAME=$(pulp container repository create --name $r | jq -r '.name')

    r_TAG=$(curl -sk -X GET $ros_REG_URL'/v2/'$r'/tags/list' | jq '.tags[]' | tr -d '"')
    printf "[INF] Processing %s:%s container image\n" "$r" "$r_TAG"
    
    echo $(pulp container remote create --name $r --url $ros_REG_URL --upstream-name $r --tls-validation false | jq -r '.pulp_created')
    ...
    let PROGRESS=(c*100)/$ROS_REPOS_CNT
    printf "====== Progress:  %.0f %% =======================================\n" "$PROGRESS"
    c=$((c+1))
done
